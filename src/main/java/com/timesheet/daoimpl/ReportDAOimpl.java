...
public class ReportDAOimpl implements ReportDAO {
	...
	public List<Report> findreport(LocalDate timesheetdate,String username)
	{
		...
		if(resultset.next())
		{
		Report report=new Report(resultset.getString("user_name"),resultset.getString("role"),resultset.getString("task_name"),
				 resultset.getInt("timesheet_id"),resultset.getInt("spend_time_hrs"),resultset.getDate("trunc(ts.timesheet_for_date)").toLocalDate(),resultset.getString("status"),resultset.getString(8),resultset.getDate("trunc(s.status_update_date)").toLocalDate());
