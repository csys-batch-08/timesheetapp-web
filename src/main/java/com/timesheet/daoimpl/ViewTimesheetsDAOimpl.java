...
public class ViewTimesheetsDAOimpl implements ViewTimesheetsDAO {
	public List<ViewTimesheets> showAllTimesheet() {
		...
		try {
			...
			while (resultset.next()) {
				ViewTimesheets timesheet = new ViewTimesheets(resultset.getString(1), resultset.getString(2),
						resultset.getInt(3), resultset.getDate(4).toLocalDate(), resultset.getInt(5),
	...
	public List<ViewTimesheets> showTimesheet(String username, String startdate, String enddate) {
		List<ViewTimesheets> timesheetlist = new ArrayList<>();
		String selectquery = "select ud.user_name,ts.timesheet_for_date,ts.spend_time_hrs,NVL(s.status,'Not Approved')as status from (status s right join timesheets ts on s.timesheet_id=ts.timesheet_id inner join user_details ud on ud.user_id=ts.user_id) where user_name='"
				+ username + "' and to_char(timesheet_for_date,'yyyy-MM-dd') between'" + startdate + "' and '" + enddate
				+ "'";
		Connection con = null;
		PreparedStatement preparestatement = null;
		ResultSet resultset = null;
		try {
			...
			resultset = preparestatement.executeQuery();
			while (resultset.next()) {
				ViewTimesheets timesheet = new ViewTimesheets(resultset.getString(1), null, resultset.getInt(3),
						resultset.getDate(2).toLocalDate(), 0, 0, resultset.getString(4));
				timesheetlist.add(timesheet);
			}
		} catch (SQLException e) {
			e.printStackTrace();
